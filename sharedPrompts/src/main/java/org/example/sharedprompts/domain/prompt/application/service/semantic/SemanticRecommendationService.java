 ...
 * Recommends role/action within category+intent; produces domain-meaningful hints
 * (why intent is preferred, why role/action fits, when fallback was used, when choice is suboptimal).
 */
@Service
public class SemanticRecommendationService {

    /**
     * Recommend role and action for (category, intent).
     * Builds structured hints from Intent Dictionary and profile (fit level, fallback, role/action rationale).
     */
    ...
    /**
     * Legacy entry point: fallback intent was not used.
     */
    public RecommendationResult recommend(
            PromptCategory category,
            ActionIntent intent,
    ...
    private List<String> buildHints(
        ...
        if (userProvidedAction == null && !actionCandidates.isEmpty() && recommendedAction != null) {
            ...
            boolean match = actionCandidates.stream().anyMatch(a -> a.key().equals(userProvidedAction.key()));
            if (!match) {
