...
public enum CloudServicesActionType implements ActionTypeInterface, StableKeyedEnum {
    ...
    CLOUD_MIGRATION("클라우드 마이그레이션", "Cloud Migration", "クラウドマイグレーション", OutputBehaviorType.STRATEGIC_PLAN),
    AWS_ARCHITECTURE("AWS 아키텍처", "AWS Architecture", "AWSアーキテクチャ", OutputBehaviorType.STRATEGIC_PLAN),
    ...
    SERVERLESS_ARCHITECTURE("서버리스 아키텍처", "Serverless Architecture", "サーバーレスアーキテクチャ", OutputBehaviorType.STRATEGIC_PLAN),
    CLOUD_COST_OPTIMIZATION("클라우드 비용 최적화", "Cloud Cost Optimization", "クラウドコスト最適化", OutputBehaviorType.STRATEGIC_PLAN),
    DISASTER_RECOVERY("재해 복구", "Disaster Recovery", "災害復旧", OutputBehaviorType.STRATEGIC_PLAN),
