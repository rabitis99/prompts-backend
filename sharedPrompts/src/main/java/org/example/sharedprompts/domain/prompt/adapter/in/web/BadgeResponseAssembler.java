...
public class BadgeResponseAssembler {
    ...
    public GeneratePromptResponse assemble(GeneratePromptResult result) {
        List<GeneratePromptResponse.BadgeDto> badgeDtos = result.badges().stream()
                .map(this::toBadgeDto)
                .toList();
    ...
    private GeneratePromptResponse.BadgeDto toBadgeDto(QualityBadge badge) {
        return new GeneratePromptResponse.BadgeDto(badge.name(), badge.getDisplayName());
    }
}
