...
public class AdminAuditLogger {
    ...
    public void logUserBlock(User admin, Long targetUserId, boolean blocked) {
        ...
        publish(admin, AuditEntityType.USER, targetUserId,
    ...
    public void logPromptDelete(User admin, Long promptId) {
        publish(admin, AuditEntityType.PROMPT, promptId,
                AuditAction.DELETE,
    ...
    public void logPromptVisibilityChange(User admin, Long promptId, boolean isPublic) {
        publish(admin, AuditEntityType.PROMPT, promptId,
                ...
                String.valueOf(!isPublic), String.valueOf(isPublic));
    }
    ...
    public void logReportProcess(User admin, Long reportId, String status, String processComment) {
        publish(admin, AuditEntityType.REPORT, reportId,
                AuditAction.REPORT_PROCESS,
                String.format("신고 처리: reportId=%d, status=%s", reportId, status),
                null, status + (processComment != null ? ": " + processComment : ""));
    }

    /**
     * 감사 로그 이벤트 발행 헬퍼 메서드
     */
    private void publish(User admin, AuditEntityType entityType, Long entityId, AuditAction action,
                         String description, Object beforeState, Object afterState) {
