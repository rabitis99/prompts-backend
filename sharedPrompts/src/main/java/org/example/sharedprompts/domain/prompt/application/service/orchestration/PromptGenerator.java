...
import org.example.sharedprompts.domain.prompt.application.service.strategy.DomainStrategyTextProvider;
...
public class PromptGenerator {
    ...
    private final DomainStrategyTextProvider domainStrategyTextProvider;
    ...
    private String buildMetaPrompt(InputRequestDto request) {
        ...
        return "You are an expert AI prompt engineer. Transform the user's input into a concise, high-quality prompt.\n\n" +

                // === HARD RULES ===
                "## Hard Rules\n" +
                "- Output ONLY the core prompt body (no role assignment, no tone/style labels, no explanations, no greetings).\n" +
                "- The role, tone, and style headers will be added separately — do NOT include them.\n" +
                "- Target approximately 450-500 characters total.\n" +
                "- Break long thoughts into 2-4 clear sentences. Each sentence focuses on one main point.\n\n" +

                // === DOMAIN-SPECIFIC GENERATION STRATEGY ===
                domainStrategyTextProvider.buildDomainStrategy(domain) +

                // === QUALITY HINTS FROM SOFT RULES ===
                domainStrategyTextProvider.buildQualityHints(domain) +

                // === CORE TASK ===
                buildEnhancedRequest(request);
    }
    ...
    private TaskDomain resolveDomain(InputRequestDto request) {
        return domainResolutionService.resolveDomainSimple(request);
    }
}
