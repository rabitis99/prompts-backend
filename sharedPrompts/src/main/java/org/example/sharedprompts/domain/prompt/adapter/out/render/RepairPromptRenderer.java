...
public class RepairPromptRenderer {
    ...
    public String renderRepair(String draft, PromptSpec spec, String failureHints) {
        ...
        if (!spec.getConstraints().getRequiredKeywords().isEmpty()) {
            sb.append("- Must include the following key terms: ")
              .append(String.join(", ", spec.getConstraints().getRequiredKeywords()))
              .append(".\n");
        }
        if (!spec.getConstraints().getProhibitedKeywords().isEmpty()) {
            sb.append("- Must avoid the following terms: ")
              .append(String.join(", ", spec.getConstraints().getProhibitedKeywords()))
              .append(".\n");
        }
        sb.append("\n");
