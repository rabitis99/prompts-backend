...
import static org.example.sharedprompts.auth.security.cors.CorsConstants.*;
...
public final class CorsConfigurationFactory {
     ...
     * - Preflight 캐시 시간: 1시간
     *
     * @param origins 허용할 origin 목록 (null이거나 비어있으면 빈 리스트로 처리)
     * @return CORS 설정
     */
    public static CorsConfiguration createDefault(List<String> origins) {
        CorsConfiguration configuration = new CorsConfiguration();
        // null 방어: null이거나 비어있으면 빈 리스트로 처리
        List<String> effectiveOrigins = (origins != null) ? origins : List.of();
