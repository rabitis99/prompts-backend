...
public class S3DownloadService {
    ...
    public S3DownloadService(
            ...
            @Value("${production.storage.s3.max-download-size-bytes:104857600}") long maxDownloadSizeBytes) {
    ...
    public Optional<Long> getContentLength(String s3Key) {
        try {
            ...
            log.error("S3 headObject failed - bucket: {}, key: {}", bucket(), s3Key, e);
            throw new S3StorageException("S3 headObject failed: " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("S3 headObject failed - bucket: {}, key: {}", bucket(), s3Key, e);
            throw new S3StorageException("S3 headObject failed: " + e.getMessage(), e);
        }
