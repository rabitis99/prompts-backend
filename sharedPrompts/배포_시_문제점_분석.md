...
- ✅ **항목 11 (트랜잭션 전파 전략)** → [`코드_문제점_분석.md`](./코드_문제점_분석.md) 항목 10과 동일 문제
  - 배포 문제 해결 시 코드 문제도 함께 해결됨

**배포 문제를 해결해도 코드 문제가 자동으로 해결되지 않는 경우:**
- ⚠️ **항목 5 (알림 시스템)** → [`코드_문제점_분석.md`](./코드_문제점_분석.md) 항목 22와 관련
  - 배포 문제: 기능 미구현 (TODO 상태)
  - 코드 문제: 설계 문제 (책임 분리)
  - 알림 기능을 구현해도 설계 문제는 별도로 해결 필요

**배포 문제를 해결해도 코드 문제와 무관한 경우:**
- 대부분의 배포 문제는 **인프라/설정/운영** 문제입니다
- 예: 환경 변수 설정, DB 마이그레이션 도구, ShedLock 테이블 생성, Redis/RabbitMQ 호스트 설정 등
- 이러한 문제들은 코드 문제와 무관합니다

### 코드 문제 → 배포 문제 해결 방향

**코드 문제를 해결하면 배포 문제도 해결되는 경우:**
- ✅ **코드 항목 10** → 배포 항목 11과 동일 (트랜잭션 전파 전략)

**코드 문제를 해결하면 배포 문제도 부분적으로 해결되는 경우:**
- ⚠️ **코드 항목 22** → 배포 항목 5와 관련 (설계 개선 시 도움)

> ⚠️ **중요**: 코드 문제만 해결해서는 배포 문제가 해결되지 않습니다!  
> 대부분의 배포 문제는 **인프라/설정/운영** 문제로, 코드 문제와는 별개입니다.  
> 일부 연관된 문제만 양방향으로 해결 가능하며, 나머지는 각각 별도로 해결해야 합니다.
...
> 🔗 **코드 연관**: [`코드_문제점_분석.md`](./코드_문제점_분석.md) 항목 22와 관련. NotificationServiceImpl의 설계 개선(전략/팩토리 패턴 적용) 시 알림 시스템 확장성 향상되어 이 배포 문제 해결에 도움됨.
...
> 🔗 **코드 연관**: [`코드_문제점_분석.md`](./코드_문제점_분석.md) 항목 10과 동일 문제. 코드에서 전파 전략을 수정하면 이 배포 문제도 해결됨.
...
## ⚠️ 코드 문제만으로는 해결되지 않는 배포 문제들

다음 배포 문제들은 **코드 문제와 무관**하며, 별도로 해결해야 합니다:

### 🔴 치명적 문제 (코드와 무관)
- ✅ **항목 1**: 필수 환경 변수 설정 (인프라/운영)
- ✅ **항목 2**: 더미 API 키 검증 (설정)
- ✅ **항목 3**: 프로덕션 프로필 활성화 (설정)
- ⚠️ **항목 4**: ShedLock 테이블 생성 (인프라)

### 🟠 중요 문제 (코드와 무관)
- ⚠️ **항목 7**: DB 마이그레이션 도구 설정 (인프라)
- ⚠️ **항목 8**: RabbitMQ 자격 증명 설정 (설정)
- ⚠️ **항목 9**: Redis/RabbitMQ 호스트 설정 (인프라)

### 🟡 개선 권장 (코드와 무관)
- ⚠️ **항목 13**: CORS 설정 검증 (설정)
- ⚠️ **항목 14**: OAuth2 리다이렉트 URL 검증 (설정)
- ⚠️ **항목 15**: 커넥션 풀 크기 최적화 (인프라/운영)
- ⚠️ **항목 16**: 로깅 설정 최적화 (운영)
- ⚠️ **항목 17**: JPA DDL Auto 설정 검증 (설정)

> 💡 **결론**: 코드 문제를 모두 해결해도 위의 배포 문제들은 별도로 해결해야 합니다.  
> 코드 문제와 배포 문제는 **대부분 독립적**이며, 각각 별도로 해결해야 합니다.
...
- 기존 [`코드_문제점_분석.md`](./코드_문제점_분석.md) 파일에 설계 및 코드 품질 관련 문제점이 정리되어 있음
- 이 문서는 배포 시 발생할 수 있는 운영상의 문제점에 집중함
- 일부 배포 문제는 코드 문제 해결로 해결 가능 (항목 11 참조)
